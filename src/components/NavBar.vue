<template>
  <div class="navbar">
    ...
    <button v-if="showToolbarAI" class="nav-btn ai-btn" @click="$emit('toggleAI')" :title="t('ai')">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2a10 10 0 1 0 10 10H12V2z"/><path d="M12 2a10 10 0 0 1 10 10h-7.5L12 9.5V2z"/><path d="M17.5 12H22v0"/><circle cx="9" cy="17" r="1"/><circle cx="13" cy="14" r="1"/><circle cx="16" cy="17" r="1"/></svg>
    </button>
    <button v-if="showToolbarExtensions" class="nav-btn" @click="$emit('toggleExtensions')" :title="t('extensions')">
    ...
    <button v-if="showToolbarBookmarks" class="nav-btn" @click="$emit('togglePanel', 'bookmarks')" :title="t('bookmarks')">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg>
    </button>
    <button v-if="showToolbarHistory" class="nav-btn" @click="$emit('togglePanel', 'history')" :title="t('history')">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
    </button>
    <button v-if="showToolbarDownloads" class="nav-btn" @click="$emit('togglePanel', 'downloads')" :title="t('downloads')">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
    </button>
    <button v-if="showToolbarReadingList" class="nav-btn" @click="$emit('toggleReadingList')" :title="t('readingList')">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>
    </button>
    <button v-if="showToolbarNotes" class="nav-btn" @click="$emit('toggleNotes')" :title="t('notes')">
...
defineProps({
  ...
  hasVideo: Boolean,
  showToolbarBookmarks: { type: Boolean, default: true },
  showToolbarReadingList: { type: Boolean, default: true },
  showToolbarHistory: { type: Boolean, default: true },
  showToolbarDownloads: { type: Boolean, default: true },
  showToolbarNotes: { type: Boolean, default: true },
  showToolbarExtensions: { type: Boolean, default: true },
  showToolbarAI: { type: Boolean, default: true },
})
defineEmits(['back', 'forward', 'reload', 'navigate', 'input', 'blur', 'update:urlInput', 'selectSuggestion', 'toggleBookmark', 'addToReadingList', 'togglePanel', 'toggleReadingList', 'toggleNotes', 'toggleExtensions', 'toggleAI', 'enterPip', 'openSettings'])
